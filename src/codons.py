...
def split_codons(dna: str) -> list[str] | None:
    ...
    if len(dna) % 3 != 0 : 
        return None
    result = [dna[i:i+3] for i in range(0,len(dna),3)]
...
def translate_codons(codons: list[str]) -> list[str]:
    ...
    result=[]
    for i in codons:
        if i.upper() not in CODON_MAP.keys():
            return None
        else:
            result.append(CODON_MAP[i.upper()])
    # FIXME: Implement the function
    return result
...
def translate_dna(dna: str) -> str:
    ...
    """
    # FIXME: Implement the function
    if split_codons(dna) is None or translate_codons(split_codons(dna)) is None:
        return None
